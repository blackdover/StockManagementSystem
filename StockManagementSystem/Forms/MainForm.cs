    ...
    // partial关键字允许将类定义拆分到多个文件中，常用于分离设计器生成的代码和用户代码
    {
        ...
        private void listViewStocks_SelectedIndexChanged_1(object sender, EventArgs e)
        {
            if (listViewStocks.SelectedItems.Count > 0) // 检查是否有选中项
            {
                ...
                {
                    DrawStockPriceChart(selectedStock.StockId); // 绘制该股票的价格图表
                }
            }
        }
        ...
        private void DrawStockPriceChart(int stockId) // 绘制股票价格图表的私有方法，接收股票ID参数
        {
            try // 尝试执行以下代码块
            {
                var stock = _stockService.GetStockById(stockId); // 根据ID获取股票对象
                var prices = _stockPriceService.GetStockPricesByStockId(stockId); // 获取该股票的所有价格数据
                ...
                // 添加数据点
                for (int i = 0; i < prices.Count; i++) // 遍历所有价格数据
                {
                    var price = prices[i]; // 获取当前价格对象
                    var date = price.TradeDate.ToShortDateString(); // 获取交易日期的短日期字符串
                    volumeSeries.Points.AddXY(date, price.Volume); // 添加成交量数据点
                    closeSeries.Points.AddXY(date, price.ClosePrice); // 添加收盘价数据点
                }
                ...
                chartArea.AxisX.LabelStyle.Font = new Font("Microsoft YaHei UI", 8); // 设置X轴标签字体
                chartArea.AxisX.MajorGrid.LineColor = Color.LightGray; // 设置X轴主网格线颜色
                chartArea.AxisX.Interval = Math.Max(1, prices.Count / 10); // 动态调整X轴标签间隔，确保不会太密集
                ...
                // 设置Y轴范围，留有10%的余量
                chartArea.AxisY.Minimum = (double)Math.Max(0, minPrice - priceRange * 0.1m); // 设置Y轴最小值，不小于0
                chartArea.AxisY.Maximum = (double)(maxPrice + priceRange * 0.1m); // 设置Y轴最大值，增加10%余量
                ...
                // 设置Y2轴范围，留有20%的余量
                chartArea.AxisY2.Minimum = 0; // 设置第二Y轴最小值为0
                chartArea.AxisY2.Maximum = (double)(maxVolume * 1.2); // 设置第二Y轴最大值，增加20%余量
